...
int convergence_layer_send_bundle(struct transmit_ticket_t * ticket)
{
	...
	//printf("Sending bundle %lu to %u.%u with ticket %p\n", ticket->bundle_number, ticket->neighbour.u8[0], ticket->neighbour.u8[1], ticket);
	...
	//		printf("Invalid bundle pointer for bundle %lu\n", ticket->bundle_number);
	...
	if( ticket->bundle->size > CONVERGENCE_LAYER_MAX_LENGTH && !(ticket->flags & CONVERGENCE_LAYER_QUEUE_MULTIPART) ) {
		...
		//	printf("Multipart bundle %lu could not be encoded, not enough memory for %u bytes\n", ticket->bundle_number, ticket->bundle->size);
		...
		//	printf("Multipart bundle %lu could not be encoded, error occured\n", ticket->bundle_number);
		...
		//	printf("Multipart bundle %lu could not be encoded, realloc failed\n", ticket->bundle_number);
	...
	//printf("convergence_layer_send_bundle\n");

//	uint32_t src=32;
	/* And send it out */
/*	struct bundle_t * dame;
	dame = (struct bundle_t *)MMEM_PTR(ticket->bundle);
	bundle_set_attr(&ticket->bundle,5,&src);*/
	//printf("Origen %d, Destino %d\n",dame->src_node,dame->dst_node);
