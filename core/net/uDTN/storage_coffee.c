...
uint8_t storage_coffee_save_bundle(struct mmem * bundlemem, uint32_t ** bundle_number_ptr)
{
	struct bundle_t * bundle = NULL;
	struct file_list_entry_t * entry = NULL;
	//struct file_list_entry_t entry2;
	...
	//printf("SAVE BUNLDE N° %d ON STORAGE COFFEE with LIFETIME %lu\n",(int)bundle->bundle_num,(uint32_t)bundle->lifetime);
	if( bundle == NULL ) {
	...
	*bundle_number_ptr = &entry->bundle_num;
	...
	//printf("SAVE BUNDLE. Hay %d guardados. N° %u. \n",bundles_in_storage,entry->bundle_num);
	return 1;
}
...
uint8_t storage_coffee_delete_bundle(uint32_t bundle_number, uint8_t reason)
{
	...
	//printf("DELETE BUNLDE N° %d ON STORAGE COFFEE, REASON %lu lifetime: %lu\n",(int)bundle_number,(uint8_t)reason,(uint32_t)bundle->lifetime);
...
//	printf("DELETE BUNDLE. Quedan %d guardados. Reason: %u. N° %u.\n",bundles_in_storage,reason,bundle_number);
	return 1;
}
