...
{
	//printf("Inicializa Discovery Basic\n");
...
{
	//printf("Llamada Discovery Basic IS NEIGHBOUR\n");
...
//	printf("Llamada SEND DISCOVERY\n");
	...
	// Llama a la función de la capa de convergencia para enviar un paquete discovery. El 13 es el length y DTN_DISCOVERY el payload.
 ...
 */
// Aparentemente en BASIC hay una devolución al send discovery de BROADCAST
uint8_t discovery_basic_is_beacon(uint8_t * msg)
{
	//printf("Llamada IS BEACON?\n");
...
void discovery_basic_neighbour_found(rimeaddr_t * neighbour)
{
//	printf("Llamada de NEIGHBOUR FOUND, Se notifica al agente\n");
	struct discovery_basic_neighbour_list_entry * entry;
...
void discovery_basic_refresh_neighbour(rimeaddr_t * neighbour)
{
//	printf("Llamada REFRESH NEIGHBOUR");
	struct discovery_basic_neighbour_list_entry * entry;
...
void discovery_basic_save_neighbour(rimeaddr_t * neighbour)
{
//	printf("Llamada SAVE NEIGHBOUR, Guarda al vecino en la cache local\n");
...
{
//	printf("Llamada BASIC DISCOVER, Start to Discover a Neighbour\n");
 ...
 */
void discovery_basic_stop_pending()
{	//printf("Llamada STOP PENDING, Descubrio un vecino, detenemos la búsqueda por un timer\n");
	discovery_pending = 0;
...
//	printf("Process DISCOVERY BASIC BEGIN\n");
