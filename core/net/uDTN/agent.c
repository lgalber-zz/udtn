...
//	CUSTODY.init();
	...
	//printf("dtn_application_rmove_event = ");
	dtn_application_remove_event  = process_alloc_event();

	//printf("dtn_application_registration_event = ");
	dtn_application_registration_event = process_alloc_event(); //141--

	//printf("dtn_application_status_event = ");
	dtn_application_status_event = process_alloc_event();

	//printf("dtn_recieve_bundle_event = ");
	dtn_receive_bundle_event = process_alloc_event();  //143--

	//printf("dtn_send_bundle_event = ");
	dtn_send_bundle_event = process_alloc_event(); //144

	//printf("submit_data_to_application_event = "); //145 Placa. En simulacion sumar 2!
	submit_data_to_application_event = process_alloc_event();

	//printf("dtn_beacon_event = ");
	dtn_beacon_event = process_alloc_event(); //146

	//printf("dtn_send_admin_record_event = ");
	dtn_send_admin_record_event = process_alloc_event();//147

	//printf("dtn_bundle_in_storage_event = ");
	dtn_bundle_in_storage_event = process_alloc_event();//148

	//printf("dtn_send_bundle_to_a_node_event = ");
	dtn_send_bundle_to_node_event = process_alloc_event();

	//printf("dtn_processing_finished = ");
	dtn_processing_finished = process_alloc_event();//150

	//printf("dtn_bundle_stored = ");
	dtn_bundle_stored = process_alloc_event();

	dtn_collector_arrived = process_alloc_event();
...
//				LOG(LOGD_DTN, LOG_AGENT, LOGL_ERR, "dtn_send_bundle_event with invalid pointer");
			...
			//	printf("dtn_send_bundle_event with invalid pointer\n");
...
//				LOG(LOGD_DTN, LOG_AGENT, LOGL_ERR, "dtn_send_bundle_event with invalid MMEM structure");
			...
			//	printf("dtn_send_bundle_event with invalid MMEM structure\n");
...
//				LOG(LOGD_DTN, LOG_AGENT, LOGL_ERR, "Unregistered process %s tries to send a bundle", PROCESS_NAME_STRING(bundle->source_process));
			...
			//	printf("Unregistered process tries to send a bundle\n");
...
//			LOG(LOGD_DTN, LOG_AGENT, LOGL_DBG, "dtn_beacon_event for %u.%u", src->u8[0], src->u8[1]);
			...
			/*if(src->u8[0]==0 && src->u8[1]==0)
			{
			process_post(collector_process, dtn_collector_arrived, NULL);
			}*/
